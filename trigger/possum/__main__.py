...
async def main(loop):
    ...
    sub = Subscriber()
    await sub.setup(d_dsn, r_dsn, username, main, mfs, mosaic, loop, options.dry_run)
    if options.run_mosaic:
        logging.info('Run mosaicking for completed tiles manually')
        await sub.run_mosaic_completed_tiles()
    else:
        await sub.consume()
